...
public class FushiSystem {
	 ...
	 * @param initialStudentCatalog the student catalog of the system
	 * @param initialTestDatabase the test database of the system
	 */
	...
	private void lookupTestScore() throws IOException {
		...
		if (student == null) {			
			...
			for(int i = 0;i<student.getExamPaper().getNumberOfItems();i++)
				System.out.println("The score of item " + (i+1) + " is: " + student.getExamPaper().getTestItem(i).getScore());
		}
	}
	...
	private void displayExamPaper() throws IOException {
		...
		if (student == null) {
			...
			for(int i=0;i<student.getExamPaper().getNumberOfItems();i++)
				System.out.println(student.getExamPaper().getTestItem(i).getTest().toString());
		}
	}
	...
	private void displayStudentCatalog() {
		...
		if(studentCatalog.getNumberOfStudents()==0) {
			stdErr.println("Empty Student Catalog!");
		} else {
			for(Student student:studentCatalog) {
				content += System.getProperty("line.separator") + student.toString();
			}			
	...
	private Student addStudent() throws IOException {
		...
		while(inputId.length()!=10) {
			stdErr.println("Invalid id! Please input a valid id!");
			System.out.print("Student id>");
			inputId = stdIn.readLine();
		}

		for(Student student:studentCatalog){
			if(student.getId().equals(inputId)) flag = 1;
		}

		while(flag == 1){
			stdErr.println("The student already exists! Please input another one!");
			System.out.print("Student id>");
			inputId = stdIn.readLine();
			for(Student student:studentCatalog){
				if(!student.getId().equals(inputId)) flag = 0;
			}
		}
		...
		while(inputName.length()==0) {
			stdErr.println("Invalid name! Please input a valid id!");
			System.out.print("Student name>");
			inputName = stdIn.readLine();
		}

		return new Student(inputId,inputName);		
	}

	/**
	 * Determine whether the input information is a number.
	 * @param str user input number
	 * @return Returns <code>true</code> if the number valid, otherwise <code>false</code>.
	 */
